...
const Properties = () => {
  ...
  const fetchProperties = async () => {
    try {
      setIsLoading(true);
      let query = supabase.from('properties').select('*').eq('status', 'active').order('created_at', {
        ascending: false
      });
      ...
      <main className="flex-1 pt-24 xs:pt-28 md:pt-32 lg:pt-36 pb-8 xs:pb-12 bg-gradient-subtle">
        <div className="container mx-auto px-3 xs:px-4 sm:px-6 lg:px-8">
            ...
            <CardContent className="p-4 xs:p-6">
              ...
              <div className="flex flex-col sm:flex-row gap-3 mb-4">
                <div className="flex-1 relative">
                  ...
                  <Input placeholder="Rechercher un bien..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} onKeyPress={e => e.key === 'Enter' && handleSearch()} className="pl-10" />
                </div>
              ...
              {showFilters && <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 pt-4 border-t border-border animate-fade-in">
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <Input placeholder="Ex: Dakar" value={cityFilter} onChange={e => setCityFilter(e.target.value)} className="pl-10" />
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <Input type="number" placeholder="100000000" value={maxPrice} onChange={e => setMaxPrice(e.target.value)} />
                  </div>
          ...
          {isLoading ? <div className="text-center py-12">
            ...
            </div> : properties.length > 0 ? <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 xs:gap-6 animate-fade-in">
              {properties.map((property, index) => <div key={property.id} className="animate-scale-in" style={{
            animationDelay: `${index * 0.05}s`
          }}>
                  <PropertyCard {...property} />
                </div>)}
            </div> : <Card className="shadow-card border-border/50">
